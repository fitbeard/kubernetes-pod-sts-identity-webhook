...
import (
	...
	cachedebug "github.com/fitbeard/kubernetes-pod-sts-identity-webhook/pkg/cache/debug"
	"github.com/fitbeard/kubernetes-pod-sts-identity-webhook/pkg/cert"
	"github.com/fitbeard/kubernetes-pod-sts-identity-webhook/pkg/containercredentials"
	"github.com/fitbeard/kubernetes-pod-sts-identity-webhook/pkg/handler"
...
func main() {
	...
	tlsCipherSuites := flag.String("tls-cipher-suites", "", "Comma-separated list of cipher suites for the server. If omitted, the default Go cipher suites will be used")
	...
	*tokenExpiration = pkg.ValidateMinTokenExpiration(*tokenExpiration)
	...
	if *tlsCipherSuites != "" {
		suites, err := pkg.ValidateTLSCipherSuites(strings.Split(*tlsCipherSuites, ","))
		if err != nil {
			klog.Fatalf("invalid tls-cipher-suites: %v", err)
		}
